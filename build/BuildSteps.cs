...
namespace Build
{
    public static class BuildSteps
    {
        ...
        {
            string authentiCodeDirectory = Path.Combine(Settings.OutputDir, Settings.SignInfo.ToSignDir, Settings.SignInfo.ToAuthenticodeSign);
            string thirdPartyDirectory = Path.Combine(Settings.OutputDir, Settings.SignInfo.ToSignDir, Settings.SignInfo.ToThirdPartySign);
            ...
            foreach (var supportedRuntime in Settings.SignInfo.RuntimesToSign)
            {
                ...
                var toSignFiles = FileHelpers.GetAllFilesFromFilesAndDirs(FileHelpers.ExpandFileWildCardEntries(toSignPaths))
                ...
                toSignFiles.ForEach(f => FileHelpers.CopyFileRelativeToBase(f, targetDirectory, sourceDir));
        ...
        public static void TestPreSignedArtifacts()
        {
            foreach (var supportedRuntime in Settings.SignInfo.RuntimesToSign)
            {
                ...
                VerifySignedBinaries(targetDir);
            }
        }
        ...
        public static void VerifySignedBinaries(string targetDir)
        {
